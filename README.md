...
- **纯净验证台上的残差收益确认**：在清洗 confounder 后，用“旧主策略 `26600` + 冻结黄金补丁”做固定状态 one-step gap 验证，指标从 `2.02` 稳定下降到 `1.81`，在离线残差补丁的基础上再次物理改善约 **10.4%**，总共比初始策略改善约**13.8%**。这说明残差补丁在局部动力学层面确实有效，收益不是日志假象，也不是奖励塑形带来的伪提升。
