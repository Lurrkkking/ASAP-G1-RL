...
class LeggedRobotMotionTracking(LeggedRobotBase):
    ...
    def _init_tracking_config(self):
        ...
        self.tracking_right_ankle_dof_indices = [
            self.dof_names.index(name) for name in ankle_dof_names if name in self.dof_names
        ]
        self.tracking_right_ankle_sigma = float(
            getattr(self.config.rewards, "tracking_right_ankle_sigma", 0.1)
        )
    ...
    def _resample_motion_times(self, env_ids):
        ...
        randomize_motion_start_train = bool(
            getattr(self.config, "randomize_motion_start_train", True)
        )
        ...
        elif (not self.is_evaluating) and (not randomize_motion_start_train):
            self.motion_start_times[env_ids] = torch.zeros(len(env_ids), dtype=torch.float32, device=self.device)
        else:
    ...
    def _reward_tracking_joint_position_right_ankle(self):
        if len(self.tracking_right_ankle_dof_indices) == 0:
            return torch.zeros(self.num_envs, dtype=torch.float, device=self.device, requires_grad=False)

        joint_pos_diff = self.dif_joint_angles[:, self.tracking_right_ankle_dof_indices]
        diff_joint_pos_dist = (joint_pos_diff**2).mean(dim=-1)
        r_joint_pos = torch.exp(-diff_joint_pos_dist / max(self.tracking_right_ankle_sigma, 1e-6))
        return r_joint_pos
