...
def _debug_print_ball_contact_nearest_bodies(env):
    if (
        not getattr(env, "debug_print_ball_contact_nearest_bodies", False)
        or env.ball_body_env_idx >= env.simulator.contact_forces.shape[1]
    ):
        return
    ...
    started = ball_has_contact & (~env.prev_ball_has_contact)
    env.prev_ball_has_contact[:] = ball_has_contact
    if not torch.any(started):
        return
    ...
    for env_id in started_ids:
        ...
        topk_nearest_dist, topk_nearest_idx = torch.topk(-dist_row, k=min(topk, dist_row.shape[0]))
        nearest_entries = []
        for _, body_idx in zip(topk_nearest_dist.detach().cpu().tolist(), topk_nearest_idx.detach().cpu().tolist()):
            nearest_entries.append(
                (
                    env.body_names[int(body_idx)],
                    round(float(dist_row[int(body_idx)].detach().cpu().item()), 4),
        ...
        logger.warning(
            ...
            "nearest_all={} nearest_upper={} nearest_target_foot={}",
            env_id,
            int(env.episode_length_buf[env_id].detach().cpu().item()),
            float(ball_contact_force[env_id].detach().cpu().item()),
            [round(float(x), 4) for x in env.ball_pos[env_id].detach().cpu().tolist()],
